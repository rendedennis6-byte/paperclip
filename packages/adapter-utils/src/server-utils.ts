...
const DIAGNOSTIC_ENV_VALUE_ALLOWLIST = new Set(["HOME", "PAPERCLIP_RESOLVED_COMMAND"]);
...
export function redactEnvForLogs(env: Record<string, string>): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(env)) {
    // Environment binding names are useful for diagnosing configuration, but
    // their values are not. Keep values only for the narrow fields deliberately
    // added by buildInvocationEnvForLogs; redact every provider/operator-defined
    // binding without trying to infer sensitivity from its name.
    redacted[key] = DIAGNOSTIC_ENV_VALUE_ALLOWLIST.has(key) ? value : REDACTED_LOG_VALUE;
  }
  return redacted;
}
